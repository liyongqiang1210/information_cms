	...
	$("#btn_delete_select").on(
			...
			function() {
				...
				if (getSelectRows.length == 0) {
					...
					openModal("删除选中角色", "确认删除选中的角色吗？", "btn-danger",
							"deleteSelectedUser()");
				}
			});
							...
							columns : [
									...
									},
									{
										...
										formatter : function(value, row, index) {
											if (value == 0) {
												return '<input type="checkbox" class="switch switch-small" name="my-checkbox" checked>';
											} else if (value == 1) {
												return '<input type="checkbox" class="switch switch-small" name="my-checkbox">';
											}
										}
									}, {
										field : 'operate',
...
// 删除角色按钮事件
function deleteRole() {
	openModal("删除角色", "确认删除选中的角色吗？", "btn-danger");
}
	...
	if (checkFromState) {
		$.ajax({
			...
			success : function(data) { // 添加成功
				closeModal();
				openPromptBox("角色添加成功", "panel-success", "icon-check-circle");
